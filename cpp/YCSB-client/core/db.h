...
class DB {
 public:
  ...
  ///
  virtual void Init(uint32_t thread_id) { }
  ///
  ...
  virtual Status Scan(const std::string &table, const std::string &key,
                   ...
                   std::vector<std::vector<Field>> &result, uint32_t thread_id) = 0;
  ///
  ...
  virtual Status Insert(const std::string &table, const std::string &key,
                     std::vector<Field> &values, uint32_t thread_id, bool doload) = 0;
  ///
  ...
  /// @return Zero on success, a non-zero error code on error.
  ///
  virtual Status Delete(const std::string &table, const std::string &key, uint32_t thread_id) = 0;
