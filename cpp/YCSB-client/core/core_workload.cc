...
void CoreWorkload::Init(const utils::Properties &p) {
  ...
  record_count_ = std::stoll(p.GetProperty(RECORD_COUNT_PROPERTY));
  ...
  if (request_dist == "uniform") {
    ...
    long long op_count = std::stoll(p.GetProperty(OPERATION_COUNT_PROPERTY));
    long long new_keys = (long long)(op_count * insert_proportion * 2); // a fudge factor
...
std::string CoreWorkload::BuildKeyName(uint32_t key_num) {
  if (!ordered_inserts_) {
    key_num = utils::Hash(key_num);
  }
  // std::string prekey = "user";
  char key[GEN_KEY_SIZE + 1];
  for (int i = 0; i < GEN_KEY_SIZE; i++) {
    key[i] = (key_num >> (8 * i)) & 0xFF;
  }
  key[GEN_KEY_SIZE] = 0;
