...
void MigrationManager::SourceMigrationManager() {
  ...
  for (int i = 0; i < storage_config.src_port_list.size(); i++) {
    ...
    inet_pton(AF_INET, storage_config.src_ip.c_str(), &(mg_init.srcAddr));
    mg_init.srcPort = storage_config.src_port_list[i];
    inet_pton(AF_INET, storage_config.dst_ip.c_str(), &(mg_init.dstAddr));
...
#if DEBUG == 1
    std::cout << "port = " << storage_config.src_port_list[i] << ", size_exp = " << size_exp << std::endl;
#endif
              ...
              for (uint32_t i = 0; i < cmds.size(); i++) {
                  ...
                  if (cmds[i].op == "GET") {
                      ...
                      if (val.has_value()) {
                          ...
                          reply_pkt_read.ver = ver | (server_type << 1) | 0; // source, fail
                      }
