...
int main(int argc, char *argv[]) {
  ...
  auto Rocksteady_migration = MigrationManager(redis_config, agent_config, server_type);
  
  if (server_type == "destination") {
    Rocksteady_migration.DestinationMigrationManager();
  }
  else if (server_type == "source_push") {
    Rocksteady_migration.SourceMigrationManager(); // This will shutdown SourceRPCServer
  }
  else if (server_type == "source_pull") {
    Rocksteady_migration.StartSourceRPCServer();
    std::this_thread::sleep_for(std::chrono::seconds(5000)); // Assume after this period, migration is finished.
    // Rocksteady_migration.FinishSourceRPCServer();
  }
