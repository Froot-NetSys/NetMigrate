...
#define CLIENT_PORT_START 5000

#define SAMPLING_PULL_TIMEOUT 1000 // microseconds
#define MCR_TIMEOUT 1000 // microseconds
...
#define SRC_THRESHOLD_CLIENT 30000 // per thread
  ...
  MigrationManager(StorageConfig storage_config, AgentConfig agent_config, std::string server_name)
    ...
    if (server_name == "destination") { 
      ...
      for (auto & port : storage_config.dst_port_list) {
        remain_size_exp.emplace_back(0);
        dst_port_idx[port] = i++;
      }
      ...
      for (uint32_t i = 0; i < agent_config.redis_scale_num * storage_config.dst_port_list.size(); i++) {
        ...
        std::vector<std::vector<std::pair<uint32_t, std::string>>> tmp_heap;
        sampling_pool.push_back(tmp_heap);
        std::vector<std::chrono::time_point<std::chrono::system_clock>> tmp_1;
        std::vector<std::chrono::time_point<std::chrono::system_clock>> tmp_2; 
        m_StartTime_Samp.push_back(tmp_1);
        m_StartTime_MCR.push_back(tmp_2);
        for (int j = 0; j < storage_config.dst_port_list.size(); j++) {
          std::vector<std::pair<uint32_t, std::string>> s_1;
          sampling_pool.back().push_back(s_1);
          m_StartTime_Samp.back().push_back(std::chrono::system_clock::now());
          m_StartTime_MCR.back().push_back(std::chrono::system_clock::now());
        }       
      }
      ...
      for (int i = agent_config.client_start_port; i < agent_config.client_start_port + agent_config.req_thread_num; i++) { 
        std::cout << i << " ";
        server_sockets.emplace_back(std::make_shared<ServerSocket>(i));
        send_migr_term.push_back(true);
      }
      std::cout << std::endl;

      // MCR initialization
      std::vector<uint32_t> mcr_tmp(agent_config.migr_thread_num, 0);
      for (int i = 0; i < storage_config.dst_port_list.size(); i++) {
        MCR.push_back(mcr_tmp);
        MCR_begin.push_back(mcr_tmp);
      }
      ...
      for (int i = 0; i < agent_config.migr_pkt_thread_num; i ++) {
        ...
        client_sockets.emplace_back(std::make_shared<BlockingClientSocket>(agent_config.dst_ip.c_str(), agent_config.dst_start_port + i, agent_config.src_start_port + i));
      }
      std::cout << std::endl;
      ...
      server_type = 0;
      src_migr_agent = std::make_shared<ShardAgent>(storage_config.src_ip, storage_config.src_port_list, agent_config.migr_thread_num);
      src_req_agent = std::make_shared<ShardAgent>(storage_config.src_ip, storage_config.src_port_list, agent_config.redis_scale_num); // For PriorityPull

      // MCR init
      for (int i = 0; i < agent_config.req_thread_num; i++) {
        send_mcr_begin.push_back(true);
      }
      std::vector<uint32_t> mcr_tmp(agent_config.migr_thread_num, 0);
...
private:
  ...
  void EnqueueDataPkt(scan_reply res, uint16_t dbPort);
  ...
  void TryAppendSPHMetadata(uint32_t thread_id, std::shared_ptr<KeyValueStoreClient> client, uint16_t dbPort, struct sph_metadata * sph_md);
  size_t TryAppendMCR(uint32_t thread_id, uint16_t dbPort, uint32_t * mcr);
  size_t TryAppendMCRBegin(uint32_t thread_id, unsigned long size_exp, uint32_t * mcr_begin);

  void DestinationRedisHandlerThread(uint32_t thread_id, std::atomic_int & mg_term);
  ...
  // For SamplingPull
  std::vector<std::vector<std::vector<std::pair<uint32_t, std::string>>>> sampling_pool;
  std::vector<std::vector<std::chrono::time_point<std::chrono::system_clock>>> m_StartTime_Samp;
  std::vector<std::vector<uint32_t>> MCR; // per req thread, a MCR array
  std::vector<std::vector<uint32_t>> MCR_begin; // per req thread
  std::vector<std::vector<std::chrono::time_point<std::chrono::system_clock>>> m_StartTime_MCR;
  std::vector<bool> send_mcr_begin; // per req thread, true stands for needing to send signal (piggybacked) to client 
  std::vector<bool> send_migr_term;
  std::vector<uint32_t> remain_size_exp;
  std::vector<uint32_t> size_exp_vec;
