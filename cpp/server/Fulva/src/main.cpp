...
int main(int argc, char *argv[]) {
  ...
  if (argc < 7) {
    std::cerr << "Usage: Rocksteady server_type[destination, source] src_redis_ip [src_redis_port,...] src_migration_agent_start_port dst_redis_ip [dst_redis_port,...] dst_migration_agent_start_port src_trans_ip dst_trans_ip agent_thread_num redis_scale_num" << std::endl;
    return 1;
  }
  ...
  auto Fulva_migration = MigrationManager(redis_config, agent_config, server_type);
  
  if (server_type == "destination") {
    Fulva_migration.DestinationMigrationManager();
  }
  else if (server_type == "source_push") {
    Fulva_migration.SourceMigrationManager(); // This will shutdown SourceRPCServer
  }
  else if (server_type == "source_pull") {
    Fulva_migration.StartSourceRPCServer();
    std::this_thread::sleep_for(std::chrono::seconds(500)); // Assume after this period, migration is finished.
    // Fulva_migration.FinishSourceRPCServer();
  }
